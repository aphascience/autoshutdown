...
class AutoOffConfig:
    ...
    def __init__(
        ...
        self.routine_first_run_time = get_first_run_time(
            self._shutdown_time, self.inactivity_threshold_mins, self.loadavg_level_mins
        )

    # TODO: maybe this is unnecessary now since all inputs are validated in parse_config()
    def _validate_config(self):
        ...
        if self.loadavg_level_mins not in LOADAVG_INDEX.keys():
            raise ValueError("'loadavg_level_mins' must be one of 1, 5 or 15")
        if not self.inactivity_threshold_mins % self.loadavg_level_mins == 0:
            raise ValueError(
                f"'inactivity_threshold_mins' must a multiple of {self.loadavg_level_mins}"
            )
...
class ShutdownTimePrompt(prompt.PromptBase[int]):
    ...
    validate_error_message = "[red]Please enter a time in 24hr format, i.e. 0000 - 2359"
...
def get_first_run_time(
    shutdown_time: datetime.time,
    inactivity_threshold_mins: int,
    loadavg_level_mins: int,
) -> datetime.time:
    """
    Gets the first run time for auto_off.py
    routine_first_run_time is "set back" from the desired shutdown
    ...
    try:
        return (
            datetime.datetime.combine(datetime.date(1, 1, 1), shutdown_time)
            - datetime.timedelta(
                minutes=(inactivity_threshold_mins - loadavg_level_mins)
            )
        ).time()
    except OverflowError:
        raise ValueError(
            "shutdown time, loadavg level and "
...
def get_inactivity_threshold_choices(
    loadavg_level_mins: int, shutdown_time: datetime.time
) -> List[str]:
    """
    ...
    return [
        ...
        for i in range(
            loadavg_level_mins,
            shutdown_time.hour * 60 + shutdown_time.minute + loadavg_level_mins + 1,
            loadavg_level_mins,
        )
    ]


def parse_version_number(version_filepath: str = DEFAULT_VERSION_FILEPATH) -> Version:
    """
...
def parse_config(old_config: dict = None) -> AutoOffConfig:
    ...
    try:
        shutdown_time = ShutdownTimePrompt().ask(
            "Choose shutdown time in 24hr format, e.g. "
        ...
        loadavg_level_mins = int(
            prompt.IntPrompt.ask(
                "Choose loadavg level (if unsure, "
        ...
        inactivity_threshold_choices = get_inactivity_threshold_choices(
            loadavg_level_mins, shutdown_time
        )
        ...
        default_inactivty_threshold = str(
            min(inactivity_threshold_choices, key=lambda x: abs(int(x) - 30))
        )
        inactivity_threshold_mins = int(
            prompt.IntPrompt.ask(
                "Choose inactivity threshold (mins), "
    ...
    cpu_idle_threshold = CPUIdlePrompt().ask(
        "Choose CPU idle threshold (if unsure, just "
        "press enter to select the default) "
        "[bold][bright_cyan](0.1)",
        default=0.1,
    )

    ssh_check = prompt.Confirm.ask(
        "Ensure no SSH connections are open before switching off your machine?",
        default=True,
    )

    default_shutdown_at_midnight = prompt.Confirm.ask(
        "Would you like your machine to shutdown at "
...
def build_cron_string(config: AutoOffConfig, auto_off_path: str) -> str:
    ...
    hr_1_minutes = ",".join(
        [str(i) for i in range(start_minute, 60, config.loadavg_level_mins)]
    )
    ...
    all_minutes = ",".join(
        [
            str(i)
            for i in range(
                int(hr_1_minutes.split(",")[-1]) + config.loadavg_level_mins - 60,
                60,
...
def activate_cron(cron_string: str, cron_filepath: str = DEFAULT_CRON_FILEPATH):
    """
    ...
    try:
        ...
        raise FileExistsError(
            f"Ensure cron file ('{cron_filepath}') is "
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(CWD, "dist/auto_off.sh"),
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nAborting auto_off configuration, leaving configuration unchanged")
