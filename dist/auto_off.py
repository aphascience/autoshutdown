    ...
    if float(loadavg_record[-1]) < config.cpu_idle_threshold:
        if len(loadavg_record) >= config.num_periods:
            if any([float(i) >= config.cpu_idle_threshold for i in
                ...
                logging.info(f"inside inactivity window - loadavg: {loadavg_record[-1]}")
                return False
            else:
                logging.info(f"shutting down machine - loadavg: {loadavg_record[-1]}")
                return True
        else:
            logging.info(f"inside inactivity window - loadavg: {loadavg_record[-1]}")
            return False
    logging.info(f"system busy - loadavg: {loadavg_record[-1]}")
    return False
