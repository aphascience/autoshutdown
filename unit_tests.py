...
import dist.auto_off as auto_off
...
class TestConfigureAutoOff(unittest.TestCase):
    ...
    def test_build_cron_string(self, mock_parse_version_number):
        ...
        with self.assertRaises(TypeError):
            activate_cron.build_cron_string("foo", "foo/auto_off.sh")

        # routine first run hour < 23
        config = activate_cron.AutoOffConfig(datetime.time(21, 00),
                                             30, 15, 0.05, True, False)
        expected = "45 20 * * * root foo/auto_off.sh 1.0.0 --inactivity_threshold_mins 30 --loadavg_level_mins 15 --cpu_idle_threshold 0.05 --ssh\n0,15,30,45 21-23 * * * root foo/auto_off.sh 1.0.0 --inactivity_threshold_mins 30 --loadavg_level_mins 15 --cpu_idle_threshold 0.05 --ssh\n"
        print(activate_cron.build_cron_string(config, "foo/auto_off.sh"))
        self.assertEqual(activate_cron.build_cron_string(config, "foo/auto_off.sh"),
                         expected)

        # routine first run hour == 22
        config = activate_cron.AutoOffConfig(datetime.time(22, 15),
                                             30, 15, 0.05, True, False)
        expected = "0,15,30,45 22 * * * root foo/auto_off.sh 1.0.0 --inactivity_threshold_mins 30 --loadavg_level_mins 15 --cpu_idle_threshold 0.05 --ssh\n0,15,30,45 23 * * * root foo/auto_off.sh 1.0.0 --inactivity_threshold_mins 30 --loadavg_level_mins 15 --cpu_idle_threshold 0.05 --ssh\n"
        self.assertEqual(activate_cron.build_cron_string(config, "foo/auto_off.sh"),
                         expected)
        ...
        config = activate_cron.AutoOffConfig(datetime.time(23, 15),
                                             30, 15, 0.05, True, False)
        expected = "0,15,30,45 23 * * * root foo/auto_off.sh 1.0.0 --inactivity_threshold_mins 30 --loadavg_level_mins 15 --cpu_idle_threshold 0.05 --ssh\n"
        self.assertEqual(activate_cron.build_cron_string(config, "foo/auto_off.sh"),
                         expected)
...
class TestAutoOff(fake_filesystem_unittest.TestCase):
    ...
    @patch("dist.auto_off.logging.info")
    def test_cpu_inactive(self, mock_logging_info):
        ...
        with patch("dist.auto_off.get_loadavg") as mock_get_loadavg:
            mock_get_loadavg.return_value = "0"
            self.assertTrue(auto_off.cpu_inactive(config,
                                                  "test_record_filepath"))
            mock_logging_info.assert_called_with("shutting down machine - loadavg: 0")
        ...
        with patch("dist.auto_off.get_loadavg") as mock_get_loadavg:
            ...
            self.assertFalse(auto_off.cpu_inactive(config,
                                                   "test_record_filepath"))
            mock_logging_info.assert_called_with("system busy - loadavg: 2")
        self.fs.remove_object("test_record_filepath")
        ...
        with patch("dist.auto_off.get_loadavg") as mock_get_loadavg:
            ...
            self.assertFalse(auto_off.cpu_inactive(config,
                                                   "test_record_filepath"))
            mock_logging_info.assert_called_with("inside inactivity window - loadavg: 0")
            self.assertTrue(auto_off.cpu_inactive(config,
                                                  "test_record_filepath"))
            mock_logging_info.assert_called_with("shutting down machine - loadavg: 0")
        self.fs.remove_object("test_record_filepath")
        ...
        with patch("dist.auto_off.get_loadavg") as mock_get_loadavg:
            ...
            self.assertFalse(auto_off.cpu_inactive(config,
                                                   "test_record_filepath"))
            mock_logging_info.assert_called_with("system busy - loadavg: 1")
            self.assertFalse(auto_off.cpu_inactive(config,
                                                   "test_record_filepath"))
            mock_logging_info.assert_called_with("inside inactivity window - loadavg: 0")
            self.assertTrue(auto_off.cpu_inactive(config,
